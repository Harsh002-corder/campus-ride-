...
export const requestSignupOtp = asyncHandler(async (req, res) => {
  ...
  if (!mailResult.sent) {
    console.error("[auth][requestSignupOtp] OTP email delivery failed", {
      email,
...
export const requestPasswordReset = asyncHandler(async (req, res) => {
  ...
  console.log("[auth][requestPasswordReset] request received", {
    email,
    nodeEnv: env.nodeEnv,
  });
  const user = await User.findOne({ email }).lean();
  ...
  if (!mailResult.sent) {
    console.error("[auth][requestPasswordReset] OTP email delivery failed", {
      email,
      reason: mailResult.reason,
      code: mailResult.code,
      nodeEnv: env.nodeEnv,
  ...
  console.log("[auth][requestPasswordReset] OTP email sent", {
    email,
    expiresAt,
  });
