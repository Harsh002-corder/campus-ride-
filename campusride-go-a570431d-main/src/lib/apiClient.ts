...
function buildTodayEarningsFromRides(rides: RideDto[]): DriverTodayEarningsDto {
...
export const apiClient = {
  ...
  rides: {
    ...
    async todayEarnings() {
      try {
        return await request<DriverTodayEarningsDto>("/rides/driver/today-earnings");
      } catch (error) {
        ...
        const fallback = await request<{ rides: RideDto[] }>("/rides/my");
        return buildTodayEarningsFromRides(fallback.rides || []);
      }
    },
